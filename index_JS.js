...
function login(event) {
    ...
    if (!csv) {
        showMessage("No users found. Please create a new account first.", false);
        return;
    }
    ...
    for (let row of rows) {
        ...
        const [name, storedEmail, storedPassword, isPaid] = row.split(",");
        ...
        if (storedEmail === email && storedPassword === password) {
            ...
            // User isPaid check can be handled on the redirected page
            // if (isPaid === "true") {
...
function handleGoogleCredentialResponse(response) {
    ...
    if (userInfo) {
        ...
        setTimeout(() => {
            window.location.href = "Welcome_Page/Welcome_Page_Index.html";  // Update to your Azure domain later
        }, 2000);
    } else {
